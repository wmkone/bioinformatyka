...
2. W panelu po lewej stronie, w części `Popular organisms` wybierz `human`. Ile rekordów zostało?
3. Skorzystaj z zaawansowanego wyszukiwania (*Advanced*) i wyszukaj białka człowieka, których nazwa zawiera `insulin`, ale nie `insulin-like`. Ile rekordów zostało? Podaj użyte zapytanie.
4. Ile rekordów pochodzi z bazy Swiss-Prot, a ile z bazy TrEMBL?
...
## Zad. 3
1. W serwisie UniProt użyj zaawansowanego wyszukiwania aby znaleźć wszystkie białka posiadające peptydy sygnałowe.
...
1. Ile rekordów białek znaleziono? Podaj zapytanie do bazy danych.
2. Zawęź wyniki wyszukiwania do białek zawierających peptydy sygnałowe o doświadczalnie potwierdzonej funkcji (`Evidence` > `Experimental`). Ile rekordów białek znaleziono? Podaj zapytanie do bazy danych.
3. Zawęź wyniki poprzedniego wyszukiwania do białek występujących u człowieka. Ile rekordów białek znaleziono? Podaj zapytanie do bazy danych.
...
3. Jakie podstawienie w sekwencji aminokwasowej rycyny niweluje jej toksyczne działanie? (`Phenotypes & Variants`)
...
6. Ogranicz liczbę wyników do ludzkich białek (`Taxon`). Ile rekordów znaleziono?
7. Ogranicz listę wyników otrzymaną w pkt. 6 do białek, których funkcja PCD została przypisana na podstawie doświadczeń laboratoryjnych (`Evidence` > `experimental evidence used in manual assertion`). Ile rekordów znaleziono?
8. Z listy wyników wybierz dowolne białko i przejdź do rekordu w bazie UniProt. Jaki jest numer dostępu tego białka?
9. W oparciu o informacje zawarte w rekordzie UniProt odpowiedz, czy białko bierze udział w procesach biologicznych innych niż PCD?
...
## Zad. 10
W serwisie UniProt wyszukaj i pobierz w formacie FASTA wszystkie białka należące do drożdży *Saccharomyces cerevisiae*. Napisz program, który wypisze identyfikatory rekordów mających identyczną sekwencję.
